...
/// Substring estável do resumo — o toggle espera ela aparecer no pane antes de fechar.
...
impl Session {
    ...
    /// `pane_id` vazio é ignorado (API às vezes omite); o pico cobre esse caso.
    ...
    pub fn summarize(&self, end_xp: u64, end_level: u8) -> Summary {
        Summary {
            agents: self.working_panes.len().max(self.peak_working),
...
mod tests {
    ...
    fn linha_com_level_up_bate_o_exemplo_da_issue() {
        ...
        assert_eq!(s.format_line(), "🐾 Sessão: 2 agentes · +1.240 XP · Nv 6 → 7 · 47 min");
        assert!(
    ...
    fn duracao_s_min_hora() {
        ...
        assert_eq!(format_duration(Duration::from_secs(3600 + 23 * 60)), "1 h 23 min");
    }
